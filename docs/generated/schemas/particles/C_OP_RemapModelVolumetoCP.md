...
[Schemas](../../schemas.md) / [particles](../particles.md) / C_OP_RemapModelVolumetoCP
...
| `0x1e0` | `m_nBBoxType` | [BBoxVolumeType_t](../!GlobalTypes/BBoxVolumeType_t.md) |  | `MPropertyFriendlyName output BBox Type` |
| `0x1e4` | `m_nInControlPointNumber` | int32 |  | `MPropertyFriendlyName input control point` |
| `0x1e8` | `m_nOutControlPointNumber` | int32 |  | `MPropertyFriendlyName output control point` |
| `0x1ec` | `m_nOutControlPointMaxNumber` | int32 |  | `MPropertyFriendlyName output max control point` `MPropertySuppressExpr m_nBBoxType != BBOX_MINS_MAXS` |
| `0x1f0` | `m_nField` | int32 |  | `MPropertyAttributeChoiceName vector_component` `MPropertyFriendlyName output CP component` `MPropertySuppressExpr m_nBBoxType != BBOX_VOLUME && m_nBBoxType != BBOX_RADIUS` |
| `0x1f4` | `m_flInputMin` | float32 |  | `MPropertyFriendlyName input volume minimum` `MPropertySuppressExpr m_nBBoxType != BBOX_VOLUME && m_nBBoxType != BBOX_RADIUS` |
| `0x1f8` | `m_flInputMax` | float32 |  | `MPropertyFriendlyName input volume maximum` `MPropertySuppressExpr m_nBBoxType != BBOX_VOLUME && m_nBBoxType != BBOX_RADIUS` |
| `0x1fc` | `m_flOutputMin` | float32 |  | `MPropertyFriendlyName output minimum` `MPropertySuppressExpr m_nBBoxType != BBOX_VOLUME && m_nBBoxType != BBOX_RADIUS` |
| `0x200` | `m_flOutputMax` | float32 |  | `MPropertyFriendlyName output maximum` `MPropertySuppressExpr m_nBBoxType != BBOX_VOLUME && m_nBBoxType != BBOX_RADIUS` |
| `0x204` | `m_bBBoxOnly` | bool |  | `MPropertyFriendlyName check full bbox only` `MPropertySuppressExpr m_nBBoxType != BBOX_VOLUME` |
| `0x205` | `m_bCubeRoot` | bool |  | `MPropertyFriendlyName cube root of volume` `MPropertySuppressExpr m_nBBoxType != BBOX_VOLUME` |
