...
[Schemas](../../schemas.md) / [server](../server.md) / CCSTeam
...
**Kind:** class · **Size:** 2080 bytes (`0x820`) · **Align:** 8 · **Module:** server

**Inherits from:** [CTeam](../server/CTeam.md)
...
106 fields (14 declared here, 92 inherited). Offsets are absolute from the object base.
...
| `0x4a8` | `m_aPlayerControllers` | CNetworkUtlVectorBase< CHandle< [CBasePlayerController](../server/CBasePlayerController.md) > > | [CTeam](../server/CTeam.md) |  |
| `0x4c0` | `m_aPlayers` | CNetworkUtlVectorBase< CHandle< [CBasePlayerPawn](../server/CBasePlayerPawn.md) > > | [CTeam](../server/CTeam.md) |  |
| `0x4d8` | `m_iScore` | int32 | [CTeam](../server/CTeam.md) |  |
| `0x4dc` | `m_szTeamname` | char[129] | [CTeam](../server/CTeam.md) |  |
| `0x560` | `m_nLastRecievedShorthandedRoundBonus` | int32 |  |  |
| `0x564` | `m_nShorthandedRoundBonusStartRound` | int32 |  |  |
| `0x568` | `m_bSurrendered` | bool |  | True when this team has surrendered the match (competitive FF vote passed). |
| `0x569` | `m_szTeamMatchStat` | char[512] |  | Match-stat summary string displayed in the team header of the scoreboard (up to 512 bytes). |
| `0x76c` | `m_numMapVictories` | int32 |  | Number of maps this team has won in the current best-of-N series. |
| `0x770` | `m_scoreFirstHalf` | int32 |  | Number of rounds won by this team during the first half. |
| `0x774` | `m_scoreSecondHalf` | int32 |  | Number of rounds won by this team during the second half. |
| `0x778` | `m_scoreOvertime` | int32 |  | Number of rounds won by this team across all overtime periods. |
| `0x77c` | `m_szClanTeamname` | char[129] |  | Team organisation/clan name displayed in the scoreboard (up to 129 bytes; set by tournament operators). |
| `0x800` | `m_iClanID` | uint32 |  | Steam group / clan ID associated with this team (used for team logo lookup). |
| `0x804` | `m_szTeamFlagImage` | char[8] |  | Short country/region flag code (up to 8 bytes) used to select the flag sprite (e.g. 'US', 'EU'). |
| `0x80c` | `m_szTeamLogoImage` | char[8] |  | Short logo image name (up to 8 bytes) used to look up the team logo spray texture. |
| `0x814` | `m_flNextResourceTime` | float32 |  |  |
| `0x818` | `m_iLastUpdateSentAt` | int32 |  |  |
